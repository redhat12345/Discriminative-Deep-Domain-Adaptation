...
class Train():
    ...
    def CalDiscriminativeLoss(self,method):
        if method=="InstanceBased":
            ...
            self.discriminative_loss = 10*self.intra_loss+ self.inter_loss
    ...
    def AIRMCoral_loss(self, h_src, h_trg):
        ...
        h_src = h_src - tf.reduce_mean(h_src, axis=0)
        h_trg = h_trg - tf.reduce_mean(h_trg, axis=0)
        cov_source = (1. / (batch_size - 1)) * tf.matmul(h_src, h_src, transpose_a=True) + 0.01 * tf.eye(int(h_src.shape[1]))
        cov_target = (1. / (batch_size - 1)) * tf.matmul(h_trg, h_trg, transpose_a=True) + 0.01 * tf.eye(int(h_trg.shape[1]))
        mat0=tf.matmul(cov_source,tf.linalg.inv(cov_target))
        eig_mat=tf.self_adjoint_eig(mat0)
        mat=tf.matmul(eig_mat[1],tf.matmul(tf.diag(tf.log1p(eig_mat[0])), eig_mat[1], transpose_b=True))
        return tf.reduce_mean(tf.square(mat))
...
def main():
    discriminative_loss_param = 1e-5 ##0.03 for InstanceBased method, 1e-5 for CenterBased method
    domain_loss_param = 8.0
    adver_loss_param=0
